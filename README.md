...
| `--oob-tx` | Lift the amateur-band transmit lockout for this run, for licensed out-of-band use. Shows a warning that must be dismissed by hand; the flag is never persisted, so it has to be passed again every launch. The warning's *Don't show again* box remembers the acknowledgement on that screen. |
